...
class NetworkAnalyser:
    ...
    def GetData(self):#get string from TCP, char array to string, split string and convert to floats. Returns float array of data with frequencies
        ...
        points = data.count(',') + 1 #amount of numbers received. One less because the last one has no comma
        data = data.split(",", points)#split it to words divided by comma
        
        for i in range (len(data) - 1):#to floats from words! For the power of Vactanilad!
            data[i] = float(data[i])

        for i in range (self.NumberOfPoints):
            freqData.append(self.center - self.span/1000 + i * (2 * self.span / self.NumberOfPoints)) #add frequency
            freqData.append(data[i]) #append Real part of S-parameter
            freqData.append(data[i+1]) #append Imaginary part of S-parameter
      
        return freqData
    
    def SavetoFile(self, filename, data_array): 
        pwd=os.getcwd()
        fh = open(pwd+'/datafiles/'+filename+'.txt',"w") #frequency RealS11 ImagS11
        for i in range(len(data_array)-3): 
            fh.write(str(data_array[i])+ ' '+str( data_array[i + 1])+' '+str(data_array[i+2])+'\n')
